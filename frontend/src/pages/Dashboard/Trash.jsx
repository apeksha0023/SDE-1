...
const Trash = () => {
  ...
  return (
    <div className="flex flex-col md:flex-row w-full h-[calc(100vh-64px)] bg-gray-100 rounded-lg shadow-sm overflow-hidden">
      {/* Sidebar - Trash Mail List */}
      <div className="md:w-1/3 w-full h-1/2 md:h-full overflow-y-auto bg-white border-b md:border-r">
        <h1 className="text-lg font-bold px-4 md:px-6 py-4 border-b text-red-600">🗑️ Trash</h1>
        {trashMails.length ? (
          <ul>
            {trashMails.map((mail) => (
              <li
                ...
                className={`px-4 md:px-6 py-4 border-b cursor-pointer hover:bg-red-50 transition ${
                  selectedMail?.id === mail.id ? 'bg-red-100' : ''
                }`}
              >
          ...
          <p className="p-4 md:p-6 text-gray-500 text-sm">No mails in trash.</p>
        )}
      </div>
